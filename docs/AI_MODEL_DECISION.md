...
> 사용자 결정(2026-05-30): **"Claude 단독으로 먼저 진행"** → 본 문서는 Claude 측 검증을 완료한 1차 산출물이며, **최종 모델 확정은 Grok 결과 합류 후**로 미룹니다(아래 [6. 최종 추천](#6-최종-추천-모델--근거) 참조).
...
**잠정 결론: 기본 모델 = Claude (단, 확정은 Grok 합류 후).**

근거(현시점):
1. **정확성** — 11문항 전수 검산에서 정답·해설·계산 불일치 0건. 본 서비스의 최대 리스크(틀린 정답 배포)를 1차 통과.
...
4. **운영 일관성** — 프롬프트(`AI_문항생성_프롬프트.md`)·서술형 채점 보조가 동일 벤더(Anthropic)로 묶이면 프롬프트 튜닝·관측이 단순해진다(`기획.md` §7도 Claude 전제).

**확정을 미루는 이유:** TASK 0의 본래 목적은 *비교*다. Grok이 환경 제약으로 미실행이므로, "Claude가 더 낫다"가 아니라 "Claude는 합격선을 넘었다"까지만 말할 수 있다. `api.x.ai` 허용 후 Grok을 동일 입력으로 돌려 §5 표를 채우고 최종 확정한다.

**다음 액션(승인 후):**
- (A) 환경 네트워크 허용목록에 `api.x.ai` 추가 → 부록 A 하니스로 Grok 3개 조건 생성 → 본 문서 §3·§5 갱신 → 최종 확정.
- (B) 또는 사용자가 외부에서 Grok 출력을 받아 붙여주면 동일 기준으로 채점.
- 확정 후에야 다음 단계(`src/lib/ai.ts`의 `generateQuestions()` 실제 연결)로 진행. **본 단계에서 mock 코드는 변경하지 않았다.**
