...
| 모델 | 상태 | 비고 |
|------|------|------|
| **Claude** (claude-sonnet-4-6) | ✅ **완료** | 자동화 스크립트(`compare-models.ts`)로 20문항 전수 검산 — 결함 0건 |
| **Grok** (grok-3) | ❌ **크레딧 소진** | 키 유효, 도메인 허용됨. xAI 팀 크레딧 한도 초과(HTTP 403) |

> **현 시점 결정(2026-06-02):** Claude 20/20 완벽 통과로 최종 확정. Grok은 xAI 크레딧 충전 후 재시도 가능하나 Claude 대비 명확한 이점이 확인되지 않는 한 교체하지 않는다.
...
개선된 프롬프트(Pass 2 강화 + 단원별 제약) 적용 후 재측정.

| 조건 | Claude 통과율 | Claude 시간 | Grok 통과율 | Grok 시간 |
|------|:------------:|:-----------:|:-----------:|:---------:|
| 곱셈/중 (5문항) | **5/5 (결함 0)** | 69.8s | ❌ 크레딧 소진 | — |
| 나눗셈/상 (5문항) | **5/5 (결함 0)** | 31.3s | ❌ 크레딧 소진 | — |
| 분수/중 (5문항) | **5/5 (결함 0)** | 67.2s | ❌ 크레딧 소진 | — |
| 들이무게/중 (5문항) | **5/5 (결함 0)** | 54.7s | ❌ 크레딧 소진 | — |
| **합계** | **20/20 (100%)** | **222.9s** | **측정 불가** | — |

**이전 대비 개선:** Pass 2 강화 + 단원별 제약 적용 전에는 나눗셈 상 난이도에서 11문항 중 1~3개 결함 발생 → 개선 후 0개.
...
근거:
1. **정확성** — 자동화 검산 20문항 전수에서 정답·해설·계산 불일치 0건. 특히 결함이 잦던 나눗셈 상 난이도(ISSUE-001)에서도 5/5 완벽 통과.
2. **교육과정 적합성** — 3학년 범위를 벗어난 개념 0건. 분수 단원에서 대분수·가분수 변환, 나눗셈에서 복합 사고형 문장제 모두 정확히 처리.
3. **형식 안정성** — JSON only·4지선다·정답 1개 규칙을 추가 후처리 없이 100% 충족.
4. **운영 일관성** — 동일 벤더(Anthropic)로 프롬프트 튜닝·관측 단순화.
5. **콘텐츠 품질** — Pass 2 자기검산 + 단원별 제약으로 결함률 0% 달성.

**Grok 비교 — 미완료 사유 및 재개 조건:**
- 미완료 사유: xAI API 키는 유효하고 도메인(`api.x.ai`)도 허용됐으나, 팀 크레딧 한도 초과(HTTP 403)로 실행 불가.
- 재개 트리거: xAI 크레딧 충전 후 `npx tsx scripts/compare-models.ts` 재실행 → §5-B 표의 Grok 열 채움.
- 재개 결과 처리: Grok이 Claude 대비 명확히 우수하면 기본 모델 교체, 아니면 Claude 유지. **코드 변경은 `generateQuestions()` 한 곳 교체로 끝난다.**
