...
type Props = {
  ...
  kind: "deploy" | "share";
  /** 구두 전달용 6자리 단축 코드 (우선 표시) */
  shortCode?: string | null;
  /** 하위 호환 — shortCode 없을 때 표시 */
  shareToken?: string;
};
...
export function NextStepBanner({ kind, shortCode, shareToken }: Props) {
    ...
    return (
      <div className="mb-4 flex items-start gap-3 rounded-xl border-2 border-sun/50 bg-sun/10 px-5 py-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-0.5 text-sm text-ink/60">
            <strong>코드만 복사</strong>로 6자리 시험 코드를, <strong>링크 복사</strong>로 응시 주소(/play)를
          ...
          {displayCode && (
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <p className="text-xs text-ink/50">
                코드:{" "}
                <span className="font-bold tracking-wider text-ink/70 tabular-nums">
                  {displayCode}
                </span>
              </p>
              <button
