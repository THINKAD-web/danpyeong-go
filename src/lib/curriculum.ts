// 초등 수학 단원 목록 (2022 개정 교육과정, EBS 기준)
// - prisma/seed.ts 의 DB 시드 원본
// - /demo 페이지가 네트워크 대기 없이 단원 목록을 즉시 보여주기 위한 정적 데이터
// 단원을 추가·변경하면 seed 를 다시 실행해 DB 와 맞춰야 한다 (term+order 가 매칭 키).

export type CurriculumUnit = { term: number; order: number; name: string; constraints: string };

// ── 3학년 수학 ────────────────────────────────────────────
// constraints: getUnitConstraints() 휴리스틱 내용을 단원별로 이전 + 단원 특화 보완
export const GRADE3_UNITS: CurriculumUnit[] = [
  {
...
// ── 4학년 수학 (신규) ─────────────────────────────────────
export const GRADE4_UNITS: CurriculumUnit[] = [
  {
...
export const MATH_UNITS_BY_GRADE: Record<number, CurriculumUnit[]> = {
  3: GRADE3_UNITS,
  4: GRADE4_UNITS,
};

/** 데모 등 공개 화면용 — 제약(constraints) 없이 단원명만 */
export function mathUnitsFor(grade: number, term: number): { term: number; order: number; name: string }[] {
  return (MATH_UNITS_BY_GRADE[grade] ?? [])
    .filter((u) => u.term === term)
    .map(({ term, order, name }) => ({ term, order, name }));
}
