...
export const GenerateInputSchema = z.object({
  ...
  // 초등 1~6학년. 중등 확장 시 상한만 조정하면 됨.
  grade: z.number().int().min(1).max(6),
  subject: z.string().min(1).max(20),
  // term: 현재 초등 수학의 1·2학기 모델. 과목에 따라 학기 개념이
  // 다를 수 있으나, 이번 PR에서는 구조를 바꾸지 않는다.
  term: z.number().int().min(1).max(2),
  count: z.number().int().min(1).max(20),
  difficulty: z.enum(["EASY", "MEDIUM", "HARD"]),
  type: z.enum(["MULTIPLE_CHOICE", "SHORT_ANSWER"]),
  // route에서 Unit.achievementStandard 를 조회해 주입. 없으면 폴백 문구 사용.
...
/** grade/subject 를 주입한 시스템 프롬프트. grade=3·subject="수학" 이면 기존 고정 문자열과 동일. */
export function buildSystemPrompt(grade: number, subject: string): string {
  return `당신은 대한민국 초등학교 ${subject} 평가 문항 출제 전문가입니다.
...
- 결과값이 음수·소수가 되는 연산 금지 (${grade}학년 범위 초과).

[금지]
- 교육과정 범위를 벗어나는 개념(예: ${grade}학년에 미등장 연산) 사용 금지.
...
/** grade/subject 를 주입한 검수 시스템 프롬프트. */
export function buildVerifySystemPrompt(grade: number, subject: string): string {
  return `당신은 초등학교 ${subject} 문항 검수 전문가입니다.
...
4. 다음 중 하나라도 해당하면 해당 문항을 배열에서 제거한다:
   ...
   - 결과가 음수·소수가 되어 ${grade}학년 범위를 벗어나는 경우
...
// ── 단원별 제약 힌트 ───────────────────────────────────────
// 현재 힌트 문구는 3학년 수학 교육과정 범위를 가정한다.
// grade !== 3 이면 힌트를 붙이지 않는다(옵션 a).
// 학년별 힌트 데이터화(Unit 제약 컬럼)는 4학년 실제 추가 시점의 별도 작업.

export function getUnitConstraints(unitName: string, grade = 3): string {
  if (grade !== 3) return "";
...
const ACHIEVEMENT_FALLBACK = "해당 단원의 핵심 계산 및 개념 이해";
...
export function buildUserPrompt(input: GenerateInput, count = input.count): string {
  ...
  const unitConstraints = getUnitConstraints(input.unitName, input.grade);
  const achievement =
    input.achievementStandard?.trim() || ACHIEVEMENT_FALLBACK;

  return `다음 조건으로 초등학교 ${input.grade}학년 ${input.subject} 단원평가 문항을 만들어 주세요.

- 학년/학기: ${input.grade}학년 ${input.term}학기
- 단원: ${input.unitName}
- 성취기준(참고): ${achievement}
- 문항 수: ${count}개
